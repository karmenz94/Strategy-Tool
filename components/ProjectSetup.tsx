...
// ⚠️ 该组件文件目前不存在：先保持注释，避免 build 失败
...
const ProjectSetup: React.FC<Props> = ({ data, onChange }) => {
  ...
  const handleChange = (field: keyof ProjectReality, value: any) => {
    let updates: ProjectReality = { ...data, [field]: value };
  ...
  // ✅ 你代码中用到了，但你贴出来的版本里没有定义：这里补齐（不改变功能）
  ...
  const updateFloorHc = (index: number, hc: number) => {
    ...
    newFloors[index] = { ...newFloors[index], headcount: hc };
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 max-w-5xl mx-auto">
              ...
              <div className="flex items-center gap-6 mb-3">
                <label className="inline-flex items-center cursor-pointer group">
                  <input
                    ...
                    onChange={() =>
                      onChange({ ...data, areaDefinition: 'NIA', targetEfficiency: 100 })
                    }
                  />
                ...
                <label className="inline-flex items-center cursor-pointer group">
                  <input
                    ...
                    onChange={() =>
                      onChange({ ...data, areaDefinition: 'Gross', targetEfficiency: 80 })
                    }
                  />
              ...
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 animate-in fade-in slide-in-from-top-2">
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {data.floors.map((floor, idx) => (
                    <div
                      key={floor.id}
                      className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm focus-within:border-orange-400 transition-colors"
                    >
                ...
                {hcMismatch && (
                  <div className="mt-4 flex items-center gap-2 text-xs font-medium text-amber-700 bg-amber-50 p-3 rounded-lg border border-amber-100">
                    ...
                    <span>
                      Warning: Floor headcount sum ({floorHcSum}) does not match Total Headcount (
                      {data.headcount}). Stacking calculations generally prioritize floor data.
                    </span>
                  </div>
                )}
              </div>
      ...
      {/* ✅ 这里是你原本报错的地方：现在把括号/注释正确关掉 */}
      {showVizSettings && (
        <div className="mt-6 p-4 rounded-lg border border-amber-200 bg-amber-50 text-amber-900 text-sm">
          <div className="font-bold mb-1">Visualization settings placeholder (temporary)</div>
          <div>
            This prototype references <span className="font-mono">VisualizationSettings</span>, but the
            component file isn’t present, so it’s disabled for build/deploy.
          </div>
          <button
          ...
          {/*
          以后你补回 VisualizationSettings.tsx 后，把上面的 placeholder 删掉，
          然后解除下面这段注释即可恢复真正功能：
