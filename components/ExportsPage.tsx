...
// ✅ 临时在本文件定义 ExportPreset，避免缺少 ExportModal 时编译失败
// 以后你补回 ExportModal.tsx 并恢复 import，也不会冲突（删掉这段即可）
type ExportPreset = 'client' | 'designer' | 'internal' | 'custom';
...
const ExportsPage: React.FC<Props> = ({ project, scenarios, activeScenarioId }) => {
  ...
  const ExportCard = ({
    ...
    <div
      className={`relative bg-white p-6 rounded-xl border transition-all flex flex-col h-full group ${
        isCustom
    ...
    >
      <div
        className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${color}`}
      >
        <Icon
          className={`w-6 h-6 ${isCustom ? 'text-indigo-600' : 'text-white'}`}
        />
      </div>
      ...
      <ul className="space-y-2 mb-6">
        {items.map((item, i) => (
          ...
          >
            <Check
              className={`w-3 h-3 ${
                isCustom ? 'text-slate-400' : 'text-emerald-500'
              }`}
            />{' '}
            {item}
          </li>
    ...
    <div className="max-w-6xl mx-auto w-full h-full flex flex-col p-6">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Generate documentation packages for different stakeholders.
        </p>
      ...
      {/* ✅ 先保留“打开导出”的功能触发：没有 ExportModal 时，用提示代替，避免 build 挂掉 */}
      {isModalOpen && (
        <div className="mt-6 p-4 rounded-lg border border-amber-200 bg-amber-50 text-amber-900 text-sm">
          <div className="font-bold mb-1">
            Export modal placeholder (temporary)
          </div>
          <div>
            Preset: <span className="font-mono">{selectedPreset}</span>
          </div>
          <div className="mt-2">
            Your project currently references an <span className="font-mono">ExportModal</span> component
            that isn’t present in the codebase, so it’s disabled for build/deploy.
            Once you add <span className="font-mono">src/components/ExportModal.tsx</span>, we can restore
            the real modal UI without changing this page’s logic.
          </div>
          <button
            className="mt-3 px-3 py-2 rounded-md border border-amber-300 bg-white hover:bg-amber-100 font-bold"
      ...
      {/*
      ✅ 以后你补回 ExportModal.tsx 后，把上面的 placeholder 删掉，
      然后解除下面这段注释即可恢复真正 modal 功能：
