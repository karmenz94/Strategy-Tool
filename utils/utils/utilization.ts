...
export const calculateMeetingMetrics = (
    ...
    roomMap.forEach((stats, key) => {
        ...
        // Find Mode (Most Frequent Size) -> P
        let modeSize = 0;
        let maxCount = 0;
        ...
        const modePct = stats.occupiedMeetings > 0 ? (maxCount / stats.occupiedMeetings) * 100 : 0;
        const topMeetingSizeStr = modeSize > 0 ? `${modeSize}p (${modePct.toFixed(0)}%)` : '-';

        // Capacity Logic & Classification
        ...
        const avgCapRatio = capacity > 0 ? avgOccupancy / capacity : 0;

        let classification: RoomClassification = 'Mixed Pattern / Review Required';
        
        if (capacity === 0 || stats.occupiedMeetings === 0) {
            classification = 'Unclassified';
        } else {
            // Standard Rounding for all values
            const C = Math.round(capacity);
            const P = Math.round(modeSize);
            const A = Math.round(avgOccupancy);
            
            const HalfCap = Math.round(0.5 * C);
            const SeventyCap = Math.round(0.7 * C);
            const EightyCap = Math.round(0.8 * C);

            // 1. UNDERUTILIZED
            // P < HalfCap AND A < HalfCap
            if (P < HalfCap && A < HalfCap) {
                classification = 'Underutilized / Size Mismatch';
            }
            // 2. REASONABLY UTILIZED
            // P in [Half, Seventy] OR A in [Half, Seventy]
            else if ((P >= HalfCap && P <= SeventyCap) || (A >= HalfCap && A <= SeventyCap)) {
                classification = 'Reasonably Utilized';
            }
            // 3. OVER UTILIZED
            // P >= EightyCap AND A >= EightyCap
            else if (P >= EightyCap && A >= EightyCap) {
                classification = 'Over Utilized';
            }
            // 4. MIXED (Default fallback)
            else {
